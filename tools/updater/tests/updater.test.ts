...
describe('updater config overlay', () => {
  ...
  test('turns artefacts on and points at the feed', () => {
    const overlay = updaterOverlay({
      TAURI_SIGNING_PUBLIC_KEY: 'pubkey-base64',
  ...
  test('takes an endpoint override, but only over https', () => {
    expect(
      updaterOverlay({
        TAURI_SIGNING_PUBLIC_KEY: 'k',
        REDROB_UPDATER_ENDPOINT: 'https://example.test/feed.json'
      })?.plugins.updater.endpoints
    ).toEqual(['https://example.test/feed.json'])
    expect(() =>
      updaterOverlay({
        TAURI_SIGNING_PUBLIC_KEY: 'k',
        REDROB_UPDATER_ENDPOINT: 'http://insecure.test/feed.json'
      })
  ...
  const fragments = [
    {
      ...
      file: 'redrob-design-arm64-0.14.0.app.tar.gz',
