 ...
 *
 * After taking anything: run `bun run check:upstream-boundary`, and set
...
function requireFetched() {
  ...
  if (!head) {
    console.error(
      `${remoteName}/${branch} is not fetched. Run: node scripts/upstream-sync.mjs fetch`
    )
    process.exit(1)
  }
  return head
...
function report() {
  ...
  process.stdout.write(
    `comparing from ${since.slice(0, 9)} (${pin.lastSyncedUpstream ? 'last synced' : 'measured base'})\n`
  )
...
function files() {
  const head = requireFetched()
  const touched = git(['diff', '--name-only', `${since}..${head}`])
    .split('\n')
    .filter(Boolean)
  ...
  process.stdout.write(
    `${touched.length} upstream path(s) changed; ${shared.length} of them exist here too.\n\n`
  )
