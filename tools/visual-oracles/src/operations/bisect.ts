...
import { headlessRenderNodes, initCanvasKit, parseFigFile } from '@redrob-design/core/io'
import { computeAllLayouts } from '@redrob-design/core/layout'
import type { SceneGraph } from '@redrob-design/core/scene-graph'
...
const { values, positionals } = parseArgs({
  ...
  options: {
    ...
    output: { type: 'string', short: 'o', default: '/tmp/redrob-design-visual-bisect' },
...
Options:
  --output DIR          Output directory (default: /tmp/redrob-design-visual-bisect)
  ...
  --root-node-id ID     RedrobDesign node whose children should be bisected
...
What it does:
  ...
  and RedrobDesign, exports that subset from both renderers, diffs the images,
...
const outputDir = values.output ?? '/tmp/redrob-design-visual-bisect'
...
async function compareSubset(indices: number[], depth: number): Promise<BisectResult> {
  ...
  const redrobDesignPath = `${outputDir}/${stem}-redrob-design.png`

  await exportFigmaSubset(figmaRootId, indices, figmaPath)
  await exportRedrobDesignSubset(indices, redrobDesignPath)

  return {
    depth,
    indices,
    names: indices.map((index) => childNames[index]),
    metrics: await diffImages(figmaPath, redrobDesignPath),
    figmaPath,
    redrobDesignPath
  }
}

async function exportRedrobDesignSubset(indices: number[], path: string): Promise<void> {
  ...
  try {
    ...
    if (!data) throw new Error(`RedrobDesign render produced no image for ${indices.join(',')}`)
...
async function writeReport(items: BisectResult[]): Promise<void> {
  ...
  for (const item of items) {
    lines.push(
      `| ${item.depth} | ${item.indices.join(',')} | ${item.metrics.above40.toFixed(3)}% | ${item.metrics.above20.toFixed(3)}% | ${item.metrics.mean.toFixed(3)} | ${item.names.map(escapePipes).join('<br>')} | [figma](${item.figmaPath}) / [redrob-design](${item.redrobDesignPath}) |`
    )
  }
