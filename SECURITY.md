...
The MCP HTTP transport (`redrobdesign-mcp-http`) binds to `127.0.0.1` by default with `eval` disabled, CORS disabled, and file access restricted to the working directory. See [MCP docs](https://redrobdesign.dev/reference/mcp-tools) for configuration.

The stdio transport (`redrobdesign-mcp`) is intended for local use only and does not apply these restrictions.
