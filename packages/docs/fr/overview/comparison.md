# Redrob Design et Penpot : architecture et performances

Redrob Design et Penpot sont des outils de design open source aux objectifs et architectures différents.
...
Redrob Design est environ onze fois plus petit. Cette différence vient surtout de l’architecture, pas uniquement du nombre de fonctionnalités.

## 2. Architecture

### Redrob Design : un seul processus client
...
Redrob Design évite latence réseau, sérialisation entre services, orchestration de conteneurs et requêtes de base de données pour les opérations courantes. Penpot vise une plateforme multiutilisateur hébergée ; Redrob Design privilégie l’édition locale à faible latence.
...
Le Tile system prépare les zones proches du Viewport et conserve jusqu’à 1 024 Textures. Redrob Design rend de nouveau toute la zone visible.
...
Redrob Design utilise directement le schéma Kiwi. Penpot doit synchroniser son modèle entre plusieurs langages.

## 5. Moteur de disposition

Redrob Design utilise Yoga WASM de manière synchrone :
...
Redrob Design utilise Yoga, avec une variante prenant Grid en charge. Penpot maintient plusieurs milliers de lignes de code de disposition dans deux langages.
...
Redrob Design lit `.fig` et Figma Clipboard directement. Penpot nécessite une voie Import ou Export distincte.

## 7. État et annulation

Redrob Design utilise des commandes inverses. Les fonctions d’avancement et de retour conservent seulement l’état nécessaire ; des lots regroupent plusieurs opérations.
...
Les changements sérialisables conviennent à la collaboration via serveur, mais augmentent la complexité. L’approche Redrob Design est plus directe pour un éditeur à processus unique.
...
5. **CSS Grid :** implémentation propre ; Redrob Design utilise une variante de Yoga avec Grid.
...
|---------|----------|-------|
| Simplicité | Redrob Design | Un processus au lieu de plusieurs services |
| Rendu | Redrob Design | Accès direct à CanvasKit |
| Code source | Redrob Design | Environ 26 000 contre 299 000 lignes |
| Compatibilité Figma | Redrob Design | Kiwi et `.fig` natifs |
| Développement | Redrob Design | TypeScript et Vue plutôt que Clojure, Rust et Docker |
| Application de bureau | Redrob Design | Tauri natif |
| Disposition | Redrob Design | Yoga plutôt que deux implémentations propres |
| Collaboration | Forces différentes | Penpot : serveur et contrôle d’accès ; Redrob Design : P2P sans hébergement |
| Auto-hébergement | Penpot | Déploiement Docker |
| Maturité de l’écosystème | Penpot | Plusieurs années en production |

Redrob Design est un éditeur compact à processus unique avec moteur CanvasKit et prise en charge native de `.fig`. Penpot est une plateforme client-serveur complète utilisant Clojure, ClojureScript, Rust, des bases de données et des services Docker. Les deux offrent la collaboration selon des modèles différents. Penpot propose un écosystème de plugins et l’export PDF ; Redrob Design fournit des scripts sans interface compatibles Figma, 90 outils AI/MCP, l’export SVG et une application de bureau.
