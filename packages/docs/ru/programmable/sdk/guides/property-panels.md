...
Панели свойств в `@redrob-design/vue` строятся прежде всего с помощью composables.
...
В интерфейсе приложения Redrob Design связанное поле в обычном состоянии показывает имя переменной на фиолетовом фоне. После начала редактирования `NumberField` показывает вычисленное числовое значение. В собственном интерфейсе редактора то же состояние можно представить иначе.
