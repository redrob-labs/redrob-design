...
Для преобразования цветов, изменения каналов и отображения ползунков без зависимости от редактора используйте [`useColorModel()`](/programmable/sdk/api/composables/use-color-model). `useOkHCL()` нужен только там, где результат должен сохраняться в Redrob Design.
