---
title: Компоненты
description: Переиспользуемые компоненты, экземпляры, наборы вариантов, переопределения, синхронизация и библиотеки в Redrob Design.
---
...
Если выделено несколько объектов, Redrob Design помещает их в новый компонент с границами по общей ограничивающей рамке.

Над компонентом отображается фиолетовая подпись со значком ромба.

## Наборы компонентов и варианты

Выделите не менее двух компонентов и нажмите <kbd>⇧</kbd><kbd>⌘</kbd><kbd>K</kbd> или <kbd>Shift</kbd> + <kbd>Ctrl</kbd> + <kbd>K</kbd>. Redrob Design объединит их в набор компонентов с пунктирной фиолетовой рамкой и внутренним отступом 40 пикселей.
...
Redrob Design сохраняет совместимые назначения текста, видимости и замены экземпляра. Если точного варианта больше нет, до подтверждения обновления будет показана верхняя левая замена. Применённое обновление можно отменить.
...
Щёлкните экземпляр правой кнопкой и выберите **Go to main component**. Redrob Design перейдёт к исходному компоненту, при необходимости переключит страницу и выделит его.
