# Redrob Design и Penpot: архитектура и производительность

Redrob Design и Penpot — открытые инструменты дизайна с разными задачами и архитектурой.
...
| Показатель | Redrob Design | Penpot |
|------------|------------|--------|
...
Redrob Design примерно в 11 раз меньше. Это следствие другой архитектуры, а не только меньшего набора возможностей.

## 2. Архитектура

### Redrob Design: один клиентский процесс
...
Redrob Design избегает сетевой задержки, сериализации между службами, управления контейнерами и запросов к базе данных для обычных операций. Penpot рассчитан на многопользовательскую платформу с серверным размещением, а Redrob Design — на локальную работу с минимальной задержкой.
...
Система использует кеш плиток, области интереса, 11 поверхностей отрисовки и глобальное изменяемое состояние через `unsafe { STATE.as_mut() }`. Кеш может хранить до 1024 текстур. Redrob Design перерисовывает видимую область целиком.

| Аспект | Redrob Design | Penpot |
|--------|------------|--------|
...
Redrob Design обеспечивает поиск по идентификатору за O(1), 29 типов объектов из схемы Kiwi, около 390 полей в `NodeChange`, строгие типы TypeScript и GUID в формате Figma `sessionID:localID`.
...
Redrob Design синхронно использует Yoga WASM:
...
Penpot поддерживает собственные реализации Flex и Grid в ClojureScript и Rust WASM. Оба движка должны выдавать одинаковый результат. Redrob Design использует Yoga, включая ответвление с поддержкой Grid; Penpot поддерживает несколько тысяч строк собственного кода компоновки на двух языках.

## 6. Форматы и Figma

Redrob Design использует бинарный формат Kiwi Figma, напрямую импортирует `.fig`, читает данные Kiwi из буфера обмена и совместим с многопользовательским протоколом Figma.
...
Redrob Design использует обратные команды: функции применения и отмены сохраняют только необходимое состояние, а несколько операций можно объединить.
...
5. **CSS Grid:** собственная реализация; Redrob Design использует ответвление Yoga с Grid.
...
|---------|--------------|---------|
| Простота | Redrob Design | Один процесс вместо нескольких служб |
| Отрисовка | Redrob Design | Прямой путь CanvasKit |
| Кодовая база | Redrob Design | Около 26 000 против 299 000 строк |
| Совместимость с Figma | Redrob Design | Нативные Kiwi и `.fig` |
| Разработка | Redrob Design | TypeScript и Vue вместо Clojure, Rust и Docker |
| Настольное приложение | Redrob Design | Tauri |
| Компоновка | Redrob Design | Yoga вместо двух собственных реализаций |
| Совместная работа | Разные преимущества | Penpot: сервер и контроль доступа; Redrob Design: P2P без размещения |
| Самостоятельное размещение | Penpot | Развёртывание Docker |
| Зрелость | Penpot | Многолетняя эксплуатация |

Redrob Design — компактный редактор в одном процессе с CanvasKit и нативной поддержкой `.fig`. Penpot — полная клиент-серверная платформа на Clojure, ClojureScript, Rust, базах данных и службах Docker. Оба поддерживают совместную работу, но используют разные модели.
