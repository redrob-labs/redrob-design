...
Redrob Design apre e salva direttamente i file `.fig`. Import ed export usano lo stesso codec binario Kiwi di Figma: 194 definizioni dello schema e circa 390 campi per ogni nodo. Salva con <kbd>⌘</kbd><kbd>S</kbd> e salva con nome con <kbd>⇧</kbd><kbd>⌘</kbd><kbd>S</kbd>.

**Copia e incolla con Figma:** seleziona gli oggetti in Figma, premi <kbd>⌘</kbd><kbd>C</kbd>, passa a Redrob Design e premi <kbd>⌘</kbd><kbd>V</kbd>. Riempimenti, contorni, disposizione automatica, testo, effetti, raggi degli angoli e reti vettoriali vengono conservati in entrambe le direzioni.
...
Redrob Design usa Skia tramite CanvasKit WASM, lo stesso motore grafico di Figma:
...
```sh
redrob-design tree design.fig              # Albero degli oggetti
redrob-design find design.fig --type TEXT  # Ricerca
redrob-design export design.fig -f png     # Esporta
redrob-design analyze colors design.fig    # Analisi colori
redrob-design analyze clusters design.fig  # Strutture ripetute
redrob-design eval design.fig -c "..."     # Figma Plugin API
```

Tutti i comandi supportano `--json`. Installazione: `npm install -g @redrob-design/cli` oppure `bun add -g @redrob-design/cli`.
...
**Web:** [app.redrob.design](https://app.redrob.design), installabile come PWA e adattato agli schermi touch.

## Caricamento alternativo da Google Fonts

Se un font non è disponibile localmente, Redrob Design lo scarica automaticamente da Google Fonts. Non è necessaria l’installazione manuale.
