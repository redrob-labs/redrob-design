...
**Copy & paste with Figma** — select nodes in Figma, <kbd>⌘</kbd><kbd>C</kbd>, switch to Redrob Design, <kbd>⌘</kbd><kbd>V</kbd>. Fills, strokes, auto-layout, text, effects, corner radii, and vector networks are preserved. Works both ways.
...
CLI: `redrob-design export design.fig -f jsx --style tailwind`
...
```sh
redrob-design tree design.fig          # Node tree
redrob-design find design.fig --type TEXT  # Search
redrob-design export design.fig -f png     # Render
redrob-design analyze colors design.fig    # Color audit
redrob-design analyze clusters design.fig  # Repeated patterns
...
```sh
redrob-design tree                     # Live document
redrob-design export -f png            # Screenshot canvas
```

All commands support `--json`. Install: `npm install -g @redrob-design/cli` (or `bun add -g @redrob-design/cli`).
...
**Web** — runs at [app.redrob.design](https://app.redrob.design), installable as a PWA on mobile with touch-optimized UI.
...
When a font isn't available locally, Redrob Design fetches it from Google Fonts automatically. No manual installation needed when opening .fig files with unfamiliar fonts.
