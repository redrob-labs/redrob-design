# Redrob Design vs Penpot: Architecture & Performance Comparison
...
Redrob Design is **~11× smaller** — and that's the whole point. It's not a simplification; it's a fundamentally different architecture.

## 2. Architecture

### Redrob Design: Monolithic Client
...
Redrob Design's single-process architecture eliminates:
...
Penpot's architecture is optimized for **multi-user server-hosted deployments**. Redrob Design is optimized for **instant local performance**.

## 3. Rendering Pipeline

### Redrob Design: TS → CanvasKit WASM (direct)
...
- **1 boundary crossing** (JS → WASM), same as Redrob Design — but with explicit serialization overhead: UUIDs split to 4×u32, transforms to 6×f32, fills/strokes binary-packed, base props batched into a 104-byte struct per shape
...
Redrob Design re-renders the full viewport every frame because CanvasKit called directly from TS is fast enough to not need tiling.

### Verdict: Rendering

| Aspect | Redrob Design | Penpot |
|--------|-------------|--------|
...
Redrob Design reuses Figma's proven schema (194 Kiwi definitions) directly in TypeScript — zero translation. Penpot maintains its own type system across Clojure/ClojureScript/Rust, requiring manual sync between all three.

## 5. Layout Engine

### Redrob Design: Yoga WASM (314 LOC)
...
Redrob Design delegates to a battle-tested library (Yoga, used by React Native on billions of devices) in 314 lines. Penpot maintains ~3,000+ LOC of custom layout code duplicated across two languages.
...
Redrob Design has a significant advantage — it can read Figma files natively and even paste Figma clipboard data. Penpot requires manual export/import and cannot open `.fig` files.
...
Redrob Design's approach is simpler and lower overhead. Penpot's approach is more suitable for collaboration (changes are serializable), but at the cost of complexity.
...
1. **Server-side collaboration** — centralized multi-user editing with WebSockets, user accounts, and access control (Redrob Design uses P2P via Trystero + Yjs — no server, but also no access control or persistence beyond the session)
...
5. **CSS Grid layout** — custom implementation (Redrob Design uses Yoga fork with grid support)
...
|-----------|--------|-----|
| **Architecture simplicity** | Redrob Design | Single process vs 5+ services |
| **Rendering performance** | Redrob Design | Direct CanvasKit vs SVG DOM (default) or binary-packed WASM |
| **Code maintainability** | Redrob Design | ~26K LOC in 1 language vs 299K in 4+ languages |
| **Figma compatibility** | Redrob Design | Native Kiwi codec vs no .fig support |
| **Developer onboarding** | Redrob Design | TS/Vue vs Clojure/Rust/Docker |
| **Desktop experience** | Redrob Design | Tauri native vs browser-only |
| **Layout engine** | Redrob Design | Yoga (proven) vs custom dual implementation |
| **Collaboration** | Tie | Penpot: server-based with access control; Redrob Design: P2P via Trystero + Yjs, zero hosting |
| **Self-hosting** | Penpot | Docker-ready vs desktop-only |
| **Ecosystem maturity** | Penpot | Years of production vs early stage |

Redrob Design is architecturally leaner — a single-process CanvasKit renderer in ~26K LOC of TypeScript, Figma-compatible by design. Penpot is a full-stack platform with ~299K LOC across Clojure, ClojureScript, Rust, and SCSS, plus a Docker service fleet. Both now offer real-time collaboration (different architectures: P2P vs server). Penpot has a plugin ecosystem and server-side PDF export; Redrob Design has Figma-compatible headless scripting, **90 AI/MCP tools**, SVG export, and a native desktop app.
